    ...
    <div class="flex justify-between items-center px-4 py-2 bg-gray-900/50 border-t border-gray-700/50">
      <div class="flex gap-2 items-center">
        <button
          @click="undo"
          :disabled="!canUndo"
          :class="canUndo ? 'text-gray-400 hover:text-white' : 'text-gray-700'"
        ...
        <button
          @click="redo"
          :disabled="!canRedo"
          :class="canRedo ? 'text-gray-400 hover:text-white' : 'text-gray-700'"
          class="text-xs px-1.5 py-0.5 rounded transition-colors"
          title="Ctrl+Shift+Z"
        >↪</button>
      </div>
      <div class="flex gap-4 text-xs text-gray-500 font-medium">
        <span>Caracteres: <span class="text-gray-300">{{ stats.chars }}</span></span>
        <span>Palabras: <span class="text-gray-300">{{ stats.words }}</span></span>
        <span>Líneas: <span class="text-gray-300">{{ stats.lines }}</span></span>
      </div>
      <div class="text-[10px] text-gray-600">TEXT TOOLKIT</div>
    </div>
  </div>
</template>
